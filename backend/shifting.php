...
if (isset($_SESSION['user_id'])) {
    ...
    $sql = "UPDATE tb_queue SET queue_status='pedding',queue_update=CURRENT_DATE WHERE queue_id=".$booking_id;
    $q = mysqli_query($conn, $sql);
    if (!$q) {
        http_response_code(500);
        echo "{message : '" . mysqli_error($conn) . "'}";
    ...
    $sql = "INSERT INTO tb_queue (patient_id,doctor_id, queue_date, queue_time_start, queue_time_end, queue_status) VALUES ('".$patient_id."','".$doctor_id."','".$queue_date."','".$queue_time_start."','".$queue_time_end."', 'shift')";
    $q = mysqli_query($conn, $sql);
    if (!$q) {
        http_response_code(500);
        echo "{message : '" . mysqli_error($conn) . "'}";
